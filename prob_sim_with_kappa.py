...
L = list(range(20,26)) # format for range(): list(range(starting length, ending length+1, increment))
mu = [0.1,0.2]
kappa = 3 
phi = 1/2 
iterations = 1000
...
for l in L: # iterates over every length desired
	for m in mu: # iterates over every mu desired
		...
		model_expected = better_expected_cms(m, l, kappa, phi) # number of expected convergent mutations from our model
