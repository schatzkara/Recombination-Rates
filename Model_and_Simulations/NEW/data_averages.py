...
def get_SNP_sim_averages(L, generations, GC, kappa, path):
		...
		for g in generations: # iterates over every number of generations the sim ran for
			for gc in GC: # iterates over every GC% the sim ran for
				for k in kappa: # iterates over every kappa the sim ran for
					...
					for x in range(g):
						data[x] = []
					with open(('averages_for_SNP_sim_' + str(l) + '_' + str(g) + '_' + str(gc) + '_' + str(k) + '.csv'), 'w', newline = '') as f: # opens the file that the data will be written to
						writer = csv.writer(f)
...
def get_ID_sim_averages(L, generations, GC, kappa, path):
	for l in L: # iterates over every length the sim ran for
		for g in generations: # iterates over every number of generations the sim ran for
			for gc in GC: # iterates over every GC% the sim ran for
				for k in kappa: # iterates over every kappa the sim ran for
					...
					for x in range(g):
						values[x] = []
						values2[x] = []
					with open(('averages_for_id_sim_' + str(l) + '_' + str(gc) + '_' + str(k) + '.csv'), 'w', newline = '') as f: # opens the file that the data will be written to
						writer = csv.writer(f)
