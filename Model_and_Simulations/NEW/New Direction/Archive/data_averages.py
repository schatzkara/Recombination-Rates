...
def get_generation_sim_averages(L, mu, generations, GC, kappa, phi, data_path, save_path):
    ...
    for x in range(generations):
        data[x] = []
    file_name = 'generation_sim_averages_' + str(L) + '_' + '{0:04}'.format(mu) + '_' + str(generations) + '_' + '{0:04}'.format(GC) + '_' + '{0:04}'.format(kappa) + '_' + '{0:04}'.format(phi)
        ...
        for filename in glob.glob(os.path.join(data_path, '*.csv')): # iterates over every .csv file in the path
            with open(filename) as d: # reads the file
                ...
                for row in range(len(file_data)): # iterates over every row of the data; the row corresponds to a generation
                    m = int(file_data[row][1]) # extracts the number of mutations on each strand from the data
                    c = int(file_data[row][2]) # extracts the number of convergent mutations from the data
                    data[m-1].append(c)
