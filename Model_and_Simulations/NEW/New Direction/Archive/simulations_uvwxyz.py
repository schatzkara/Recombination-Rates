...
def mutate(current_nucleotide, mu, u,v,w,x,y,z, force_mutation):
	nucleotides = ['A', 'T', 'G', 'C']
	...
	if force_mutation:
		weights_A = [0, w/(w+u+x), u/(w+u+x), x/(w+u+x)]
		weights_T = [w/(w+y+v), 0, y/(w+y+v), v/(w+y+v)]
		weights_G = [u/(u+y+z), y/(u+y+z), 0, z/(u+y+z)]
		weights_C = [x/(x+v+z), v/(u+y+z), z/(u+y+z), 0]
	else:
		weights_A = [(1-mu), w, u, x]
		weights_T = [w, (1-mu), y, v]
		weights_G = [u, y, (1-mu), z]
		weights_C = [x, v, z, (1-mu)]
...
def mutation_sim(n, L, mu, generations, u,v,w,x,y,z):
	...
	for child in range(n):
		...
		for site in range(len(t)): # goes through each nucleotide and mutates it or keeps it the same
			for g in generations:
				...
				t[site] = mutate(current_nucleotide, mu, u,v,w,x,y,z, False)
		t = ''.join(t)
...
def efficient_mutation_sim(L, mu, generations, u,v,w,x,y,z):
	...
	for y in range(L): # builds a random ancestor strand of length L
		...
		for child in range(2):
			...
			for g in range(generations):
				...
				t[y] = mutate(current_nucleotide, mu, u,v,w,x,y,z, False)
			t = ''.join(t)
			strains[child] = t
...
def generation_sim(L, mu, generations, GC_prop, u,v,w,x,y,z):
	...
	for g in range(generations): # adds m mutations for each generation
		for child in range(2): # adds m mutations to each child strand
			...
			for x in range(m): # mutates the appropriate number of sites for a generation
				...
				current_nucleotide = t[site]
				t[site] = mutate(current_nucleotide, mu, u,v,w,x,y,z, True)
...
def identity_sim(L, mu, generations, GC_prop, u,v,w,x,y,z):
...
def id_matrix_sim(n, L, mu, generations, u,v,w,x,y,z):
	ancestor = '' # ancestor DNA strand
...
def c_q_sim(n, L, mu, generations, u,v,w,x,y,z):
	strains = n*[None] # list of the child DNA sequences
